...
import EditOrder from "./EditOrderStatus";
import {
  useDeleteOrderStatusMutation,
  useGetAllOrderStatusQuery,
} from "../../../../redux/api/orderStatus/orderStatusAPI";

// Table head constant
const columns = [
  { id: 1, label: "ID", minWidth: 250 },
  { id: 2, label: "Name", minWidth: 120 },
  {
    id: 3,
    label: "Status",
    minWidth: 120,
...
const OrderStatusContainer = ({ keyword }) => {
  ...
  useEffect(() => {
    ...
    if (data) {
      const orderStatusesCopy = [...data?.orderStatuses];
      const orders = orderStatusesCopy?.filter((order) =>
  ...
  useEffect(() => {
    if (isDeleteError) {
      toast.error(orderStatusError?.message || "User not deleted.");
    }
    if (isDeleteSuccess) {
      toast.success(orderStatusData?.message);
    }
  }, [
    orderStatusData?.message,
  ...
  return (
    <Container>
        ...
        >
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell style={{ textAlign: "right" }}>
                          <svg
                            onClick={() => handleDelete(orderStatus?._id)}
          ...
          {orderStatuses.length && (
            <TablePagination
              ...
              component="div"
              count={Number(orderStatuses?.length)}
      ...
      <Modal open={edit} onClose={() => setEdit(false)}>
        <EditOrder orderStatus={orderForEdit} onClose={() => setEdit(false)} />
      </Modal>
    </Container>
  );
};

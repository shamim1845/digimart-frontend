...
const CategoriesContainer = ({ keyword }) => {
  ...
  // handle client side Categories filtering
  useEffect(() => {
    if (keyword) {
      setPagination({
        page: 0,
    ...
    if (data) {
      ...
      const categories = categoriesCopy?.filter((category) =>
  ...
  // handle delete category mutation
  const [
    deleteCategory,
    {
  ...
  // handle Category successfully deleted or occure an error
  useEffect(() => {
    if (isDeleteError) {
      toast.error(categoryError?.message || "Category not deleted.");
    }
    if (isDeleteSuccess) {
                      ...
                      >
                        ...
                        <TableCell style={{ fontSize: "1.3rem" }}>
                          {category?.ancestors?.length > 0 && (
                            <span>
                              [
                              {category?.ancestors?.map((parent, i) =>
                                i + 1 === category?.ancestors?.length
                                  ? parent?.name
                                  : parent?.name + ", "
                              )}
                              ]

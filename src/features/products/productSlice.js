...
export const fetchAsyncProducts = createAsyncThunk('products/fetchAsyncProducts', async(page) => {
...
export const  fetchAsyncProductForFilter = createAsyncThunk('products/fetchAsyncProductForFilter', async({keyword, category, pricestart, priceend}) => {
...
export const fetchAsyncProductsDetails = createAsyncThunk('products/fetchAsyncProductDetails', async(id) => {
...
const productSlice = createSlice({
    ...
    reducers: {
        ...
        addCartItem: (state, {payload}) => {
          ...
          const ExistCartItem = state.cartItem && state.cartItem.find((item) =>  (
        ...
        deleteCartItem: (state, {payload}) => {
         const deletedCartItem = state.cartItem.filter((item) => (
                item.product._id !== payload.product._id 
            ))
        ...
        addFavouriteItem: (state, {payload}) => {
          ...
          const ExistFavouriteItem = state.favouriteItem && state.favouriteItem.find((item) =>  (
                         item.product._id === payload.product._id )  )  
            ...
            localStorage.setItem('favourite-item', JSON.stringify(state.favouriteItem))
        },
        deleteFavouriteItem: (state, {payload}) => {
         const deletedFavouriteItem = state.favouriteItem.filter((item) => (
                item.product._id !== payload.product._id 
            ))
...
export const {addKeyword, addCategory, addCartItem, deleteCartItem, addFavouriteItem, deleteFavouriteItem} = productSlice.actions;
